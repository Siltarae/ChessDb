...
- **이 작업의 책임**: 반수별 어노테이션 선택 UI와 draft 상태 저장 규칙을 구현합니다.
- **이번 작업에서 하지 않는 것**: 브라우저 저장소 자동 저장이나 정식 기보 상세 화면의 조회/수정은 후속 태스크로 남깁니다.
- **경계 메모**:
  - 이 문서는 어노테이션 기호 하나를 선택해 저장하는 책임만 다룹니다. 코멘트 입력과 별도 액션을 유지합니다.

## 🎯 1. 작업 목표

- **최종 상태**: 사용자가 특정 반수에서 평가 기호 버튼을 누르면 해당 반수에 어노테이션이 즉시 연결됩니다.
- **이번 작업의 최소 결과물**:
...
- **핵심 구현 대상**:
  ...
  - 같은 기호를 다시 누르면 해제할지 유지할지 규칙을 문서에서 먼저 고정하고 테스트로 박습니다.
- **데이터 모델 해석**:
  - `draft.moves[plyIndex].annotation`을 반수별 평가 기호 저장 위치로 해석합니다.
  - 어노테이션은 자유 텍스트가 아니라 고정된 리터럴 집합 중 하나여야 합니다.
- **외부 의존성**:
  - `react`
  - `zustand`
...
- **import/export 규칙**:
  ...
  - `draft-store.ts`는 어노테이션 타입을 중앙에서 고정합니다.
- **권장 네이밍**:
...
1. `draft-store.ts`에 어노테이션 타입과 `updateMoveAnnotation` 액션을 추가합니다.
