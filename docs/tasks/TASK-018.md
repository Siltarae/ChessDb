...
- **최종 상태**: 사용자가 되돌리기 버튼이나 단축키를 누르면 직전 반수 시점의 보드가 즉시 복구됩니다.
- **이번 작업의 최소 결과물**:
...
- **핵심 구현 대상**:
  ...
  - `history-controls.tsx`는 버튼 클릭과 `ArrowLeft` 단축키를 `goToPreviousPly`에 연결합니다.
- **데이터 모델 해석**:
...
- **이름별 사용 의도와 적용 시점**:
  ...
  - `goToPreviousPly`는 키보드 입력과 버튼 입력이 같은 액션을 공유하도록 만들 때 사용합니다.
- **인수 이름 가이드**:
...
- 단축키 리스너를 붙이더라도 입력 필드 포커스 중에는 실행되지 않도록 분기합니다.

## 🧪 5. 검증 시나리오 및 단언

1. **정상 시나리오: 한 단계 되돌리기**
   - 세 번째 반수 시점에서 `goToPreviousPly`를 호출한다.
   - 두 번째 반수의 보드와 현재 턴이 화면에 반영된다.
...
3. `history-controls.tsx`에 Undo 버튼과 키보드 이벤트를 연결합니다.
