...
- **이 작업의 책임**: 결과(`1-0`, `0-1`, `1/2-1/2`, `*`)와 종료 사유를 입력해 draft 메타데이터에 반영합니다.
- **이번 작업에서 하지 않는 것**: 대국 날짜 입력은 `[TASK-027]`, 정식 저장 API 전송은 `[TASK-025]`에서 처리합니다.
- **경계 메모**:
...
- **성공 기준 (AC)**:
  - 결과 값은 허용된 PGN 결과 리터럴 중 하나만 저장된다.
  - 종료 사유는 고정된 옵션 목록에서 선택된다.
