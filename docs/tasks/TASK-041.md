...
- **현재 상태 요약**: 상세 페이지에 보드 자리만 있어도 수순 목록을 함께 보여주는 위젯은 아직 없습니다.
- **이 작업의 책임**: 상세 화면에 수순 목록 위젯을 배치하고 저장된 수순을 읽어 표시합니다.
...
- **최종 상태**: 상세 뷰에서 저장된 수순 목록을 함께 확인할 수 있습니다.
- **이번 작업의 최소 결과물**:
...
- **성공 기준 (AC)**:
  ...
  - 코멘트와 어노테이션 표시의 기준점이 되는 반수 인덱스가 유지된다.
...
- `feat: 기보 상세 화면에 수순 목록 위젯을 추가 (TASK-041)`
