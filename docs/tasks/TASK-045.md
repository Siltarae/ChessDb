...
**후속 작업**: `[TASK-046]` (어노테이션 수정과 자동 저장), `[TASK-103]` (정식 기보 수순 수정)  
...
- **이번 작업에서 하지 않는 것**: 어노테이션 수정과 수순 수정은 별도 태스크에서 다룹니다.
- **경계 메모**:
  - 코멘트 수정과 저장만 다룹니다.
...
- **성공 기준 (AC)**:
  ...
  - 코멘트 수정은 즉시 자동 저장된다.
  - 수순 수정 Task와 충돌하지 않도록 반수 연결이 유지된다.
...
- 수순 데이터 자체는 이번 태스크에서 수정하지 않습니다.
- 이후 수순 수정 시 코멘트 보존/삭제 규칙은 `[TASK-103]`에서 확정합니다.
...
- [ ] 코멘트 수정이 가능하다.
- [ ] 자동 저장이 동작한다.
- [ ] 수순 수정 Task와 충돌하지 않는 반수 연결을 유지한다.
