# 📋 개별 작업 지침서: 기보 및 초안 데이터 스키마 정의 (TASK-063)
...
- **경계 메모**:
  - 공통 저장 스키마와 타입만 다룹니다.
  - `FEATURE-004` 이전에는 사용자 저장소 선택을 다루지 않고 시스템 기본 저장소 식별자만 저장 계약에 포함합니다.
...
- **성공 기준 (AC)**:
  ...
  - 영속성 메타데이터가 포함된다.
  - `repositoryId`는 시스템 기본 저장소를 가리킬 수 있지만, 저장소 목록/생성/삭제 구조는 포함하지 않는다.
...
- **데이터 모델 해석**:
  - `GameRecord`는 `repositoryId`, `initialState`, `history`, `metadata`, `createdAt`, `updatedAt`를 포함하는 최상위 저장 모델입니다.
  - `repositoryId`는 `FEATURE-004` 전까지 시스템 기본 저장소 ID를 사용합니다.
  - 저장소 이름, 생성일, 목록 정렬 같은 저장소 관리 필드는 `FEATURE-004`에서 별도 모델로 확장합니다.
- **외부 의존성**:
  - `zod`
