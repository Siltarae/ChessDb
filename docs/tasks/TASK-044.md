...
- **현재 상태 요약**: 상세 화면에서 저장된 착수 평가 기호를 읽어 보여주는 별도 기준이 없습니다.
- **이 작업의 책임**: 정식 저장된 기보의 어노테이션을 상세 뷰에서 조회하고 표시합니다.
...
- **성공 기준 (AC)**:
  ...
  - 코멘트 조회 규칙과 함께 동작하지만 별도 데이터로 분리된다.
...
- **외부 의존성**:
  - `@/shared/ui/badge`
- **import/export 규칙**:
  - 조회 전용 컴포넌트에서 수정 action을 import하지 않습니다.
- **권장 네이밍**:
...
- `feat: 정식 기보 어노테이션 조회와 표시를 추가 (TASK-044)`
