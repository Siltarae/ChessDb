...
- **현재 상태 요약**: 상세 화면에서 수순 목록은 볼 수 있지만 저장된 코멘트를 읽어 보여주는 기준이 없습니다.
- **이 작업의 책임**: 정식 저장된 기보의 반수별 코멘트를 상세 화면에서 조회하고 표시합니다.
