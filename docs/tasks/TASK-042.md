...
- **현재 상태 요약**: 상세 화면에 보드와 수순 목록은 있어도 둘이 같은 시점을 가리키도록 동기화하는 상호작용이 없습니다.
- **이 작업의 책임**: 수순 목록의 특정 반수를 선택하면 상세 보드가 해당 시점으로 이동하도록 연결합니다.
...
- **최종 상태**: 사용자가 상세 화면에서 특정 수를 선택하면 해당 시점 보드 상태가 보입니다.
- **이번 작업의 최소 결과물**:
...
- 입력용 합법 수 강조 로직을 상세 화면에 도입하지 않습니다.
