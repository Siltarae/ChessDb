...
import { executePromotion, isPromotionSquare } from './promotion-engine.js';
...
describe('PromotionEngine', () => {
  ...
  describe('applyPromotion', () => {
    it('출발 칸이 비어 있으면 원본 상태를 그대로 반환해야 한다', () => {
      ...
      expect(executePromotion(SQUARE.E7, SQUARE.E8, state)).toBe(state);
    });
    ...
    it('프로모션 정보가 없으면 기본값으로 퀸 승격을 적용해야 한다', () => {
      ...
      const nextState = executePromotion(SQUARE.E7, SQUARE.E8, state);
    ...
    it.each([
      ...
      (_name, pieceType) => {
        ...
        const nextState = executePromotion(SQUARE.D2, SQUARE.D1, state, pieceType);
    ...
    it('상대 기물을 잡으면서 마지막 랭크에 도달한 경우에도 프로모션을 적용해야 한다', () => {
      ...
      const nextState = executePromotion(SQUARE.G7, SQUARE.H8, state, PIECE_TYPE.BISHOP);
    ...
    it('마지막 랭크가 아니면 원본 상태를 그대로 반환해야 한다', () => {
      ...
      expect(executePromotion(SQUARE.E6, SQUARE.E7, state)).toBe(state);
    });
    ...
    it('허용되지 않은 승격 기물 타입이면 원본 상태를 그대로 반환해야 한다', () => {
      ...
      expect(executePromotion(SQUARE.E7, SQUARE.E8, state, PIECE_TYPE.KING)).toBe(state);
      expect(executePromotion(SQUARE.E7, SQUARE.E8, state, PIECE_TYPE.PAWN)).toBe(state);
    });
    ...
    it('프로모션 후에도 원본 state와 board는 오염되지 않아야 한다', () => {
      ...
      const nextState = executePromotion(SQUARE.G7, SQUARE.H8, state, PIECE_TYPE.QUEEN);
